...
namespace NppPluginNET
{
    ...
    {
        #region " Fields "
        internal const string PluginName = "SureScripts";
        ...
        static internal void CommandMenuInit()
        {
            ...
            SetCommand(0, "Transform", transformSureScriptsMsg, new ShortcutKey(true,false,true,Keys.T));
        ...
        static void transformSureScriptsMsg()
        {
            // Say hello now :
            // Scintilla control has no Unicode mode, so we use ANSI here (marshalled as ANSI by default)
            string message = GetCurrentSelectedText(GetCurrentScintilla());
            // Open a new document
            if (!string.IsNullOrEmpty(message) && message.StartsWith("UNA"))
            {
                try
                {
                    string transformedMsg = InformationExtractor.instance(message).informationToPrint();
                    Win32.SendMessage(nppData._nppHandle, NppMsg.NPPM_MENUCOMMAND, 0, NppMenuCmd.IDM_FILE_NEW);
                    Win32.SendMessage(GetCurrentScintilla(), SciMsg.SCI_SETTEXT, 0, transformedMsg);
                }
                catch (Exception)
                {
                }
        ...
        public static string GetCurrentSelectedText(IntPtr curScintilla)
        {
            int length = (int)Win32.SendMessage(curScintilla, SciMsg.SCI_GETLENGTH, 0, 0) + 1;
            StringBuilder sb = new StringBuilder(length);
            Win32.SendMessage(curScintilla, SciMsg.SCI_GETSELTEXT, length, sb);
            return sb.ToString();
        }

        static void helloFX()
        {
            transformSureScriptsMsg();
